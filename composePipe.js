...
// Iterate right to left using reduce
var compose = function(){
  // Copy the arguments to the compose function into a new arr
  var args = Array.prototype.slice.call(arguments, 0);

  // For each function argument,
  //   return a new function that's composed of
  //   all the functions we've composed so far
  return args.reduceRight((composedFunc, func) => {
    return (args) => {
      return func(composedFunc(args))
    }
  })
};

// Left to right composition:
// Iterate left to right using reduce
var pipe = function(){
  // Copy the arguments to the compose function into a new arr
  var args = Array.prototype.slice.call(arguments, 0);
  ...
  //   return a new function that's composed of
  //   all the functions we've composed so far
  return args.reduce((composedFunc, func) => {
    return (args) => {
      return func(composedFunc(args))
    }
  })
