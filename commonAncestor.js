...
Tree.prototype.getClosestCommonAncestor = function(node1, node2){
  // Stores the closest common ancestor
  var cca = null;

  // Recursively traverse the tree
  (function traverse(tree) {
    // If the both nodes are descendants of current node, continue
    // searching children
    if (tree.isDescendant(node1) && tree.isDescendant(node2)) {
      // Update closest common ancestor
      cca = tree;

      // Recurse on each child so long as node1 and node2 are
      // stil descendants
  ...
  // Return this if node1 and node2 are the same (root)
  // otherwise return whatever cca we found if any
  return node1 === node2 ? this : cca;
}

Tree.prototype.getAncestorPath = function(node){
  var path = [];

  // Use an IIFE to recursively traverse the tree
  (function traverse(tree) {
    // So long as node is a descendent, keep descending
    // the tree, adding each node to the path until we
    // reach node
    if (tree.isDescendant(node)) {
      ...
      // We've reached the target node, add it to
      // path and don't traverse any further
      path.push(tree);
    }
  })(this);

  // Return the path or null if path is empty
